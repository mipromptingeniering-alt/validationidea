...
def sync_idea_to_notion(idea):
    try:
        ...
        properties = {
            ...
            # ScoreMoney omitido aqui — se añade condicionalmente abajo
            "Date":        {"date": {"start": idea.get("fecha") or datetime.now().isoformat()}},
        }

        # ScoreMoney solo si existe en Notion DB
        try:
            db_check = requests.get(f"https://api.notion.com/v1/databases/{DATABASE_ID}", headers=HEADERS, timeout=10)
            if db_check.status_code == 200 and "ScoreMoney" in db_check.json().get("properties", {}):
                properties["ScoreMoney"] = {"number": int(money)}
            else:
                print("⚠️ ScoreMoney no existe en Notion DB — omitido")
        except Exception:
            pass

...
# ── Fallback Gemini (auto-activado si Groq cae) ───────────────────────────────
_groq_caido = False
_groq_ultimo_fallo = 0

def _llamar_gemini(messages, max_tokens=3000):
    try:
        import google.generativeai as genai
        genai.configure(api_key=os.environ.get("GEMINI_API_KEY",""))
        modelo = genai.GenerativeModel("gemini-2.0-flash")
        prompt_completo = " ".join([m["content"] for m in messages])
        r = modelo.generate_content(prompt_completo)
        print("   ✅ Gemini OK (fallback activo)")
        return r.text
    except Exception as e:
        print(f"   ❌ Gemini fallback fallo: {e}")
        return None
...
# ── Fallback Gemini ────────────────────────────────────────────────────────────
def _llamar_gemini_fallback(messages, max_tokens=3000):
    try:
        import google.generativeai as genai
        genai.configure(api_key=os.environ.get("GEMINI_API_KEY", ""))
        modelo = genai.GenerativeModel("gemini-2.0-flash")
        prompt_full = chr(10).join(m['content'] for m in messages)
        r = modelo.generate_content(prompt_full)
        print("   ✅ Gemini fallback OK")
        return r.text
    except Exception as e:
        print(f"   ❌ Gemini fallo: {e}")
        return None
...
def llamar_groq(prompt, max_tokens=3000, sistema=None):
    global _groq_caido, _groq_ultimo_fallo
    # Reintentar Groq cada 60 min
    if _groq_caido and (time.time() - _groq_ultimo_fallo) > 3600:
        _groq_caido = False
        print("   🔄 Reintentando Groq (60min retry)...")
    pesos    = _cargar_pesos()
    ...
    print("   ⚠️ Groq agotado, activando Gemini fallback...")
    return _llamar_gemini_fallback(messages, max_tokens)
